...
function buildPrompt(headlines, startNum) {
  ...
  lines.push('・スライド全体の高さを100%としたとき、上端0%～4%の帯は背景色のみの空白エリアとしてください。');
  lines.push('・上端4%～下端94%（高さ90%）がコンテンツ配置エリアです。すべての要素（図解・アイコン・ボックス・キーワード等）はこのエリアに収めてください。');
  lines.push('・コンテンツ配置エリアは上から下へ均等に使い切るようにデザインしてください。コンテンツ量が少ない場合でも、図解・強調ボックス・区切り装飾などを追加してエリア全体を埋めてください。');
  lines.push('・コンテンツ配置エリアの先頭（上端4%直下）から要素の配置を開始し、上端4%～20%の範囲が空白になることは禁止します。');
  lines.push('・下端94%～100%の帯は背景色のみの空白エリアとし、いかなる要素も配置しないでください。');
  lines.push('');
  lines.push('【禁止パターン】');
  lines.push('・上端4%～20%の範囲が空白・背景色のみになっている');
